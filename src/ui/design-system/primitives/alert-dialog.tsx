/**
 * ALERT DIALOG COMPONENT - DESIGN SYSTEM COMPLIANT
 *
 * Uses ONLY CSS variables from design tokens.
 * NO hardcoded overlay colors.
 * Accessible alert dialog for confirmations and critical actions.
 *
  ...
  <AlertDialogPortal>
    ...
    <AlertDialogPrimitive.Content
      ...
      className={cn(
        ...
        // `translate-x-[-50%]`. Logical `start-[50%]` breaks centering in RTL
        // (start→right, but translate-x stays physical → dialog shifts left).
        // eslint-disable-next-line custom/no-physical-direction -- physical pair required for correct RTL centering
        "fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%]",
        "gap-4 border border-border bg-card p-6 shadow-lg rounded-xl",
        "duration-200",
        "data-[state=open]:animate-in data-[state=closed]:animate-out",
        "data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
